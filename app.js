...
(function () {
  ...
  async function fetchPage(page) {
    const params = new URLSearchParams({
      ...
      fields: 'id,title,artist_display,date_end,style_title,image_id,is_public_domain',
      limit: 100,
      page,
    });
    ...
    return data.data
      ...
      .map((d, i) => ({
        id:      `artic-p${page}-${i}`,
        title:   d.title || '無題',
        artist:  (d.artist_display || '作者不詳').split('\n')[0].split(',')[0].trim(),
        year:    d.date_end || 0,
  ...
  async function initialLoad() {
    showLoadingCard();
    try {
      allPaintings = await fetchPage(1);
      updateFilterOptions();
      removeLoadingCard();
      initDeck();
      // バックグラウンドで追加取得
      ...
      removeLoadingCard();
      showToast('読み込み失敗。リロードしてください。');
    }
  }

  function showLoadingCard() {
    const c = document.createElement('div');
    c.className = 'card cur'; c.id = 'lc';
    c.innerHTML = `<div style="display:flex;flex-direction:column;align-items:center;gap:14px;color:var(--ink-l)"><div class="ph-spinner"></div><div style="font-size:11px;letter-spacing:.15em;font-style:italic">名画を読み込み中</div><div style="font-size:9px;letter-spacing:.1em;opacity:.5">Art Institute of Chicago</div></div>`;
    dw.appendChild(c);
  }
  function removeLoadingCard() {
    const c = document.getElementById('lc'); if (c) c.remove();
  }
  ...
  function clearDeck() { cards.forEach(c => c.el.remove()); cards = []; pointer = 0; }
  ...
  function initDeck() {
    ...
    for (let i = 0; i < Math.min(3, queue.length); i++) {
      ...
      dw.appendChild(el); cards.push({ el, data: queue[i] });
    }
  ...
  function makeCard(p, idx, total) {
    const card = document.createElement('div'); card.className = 'card'; card._p = p;
    const num  = document.createElement('div'); num.className = 'card-num'; num.textContent = `${idx + 1} / ${total}`;
    const stag = document.createElement('div'); stag.className = 'style-tag'; stag.textContent = p.style || '';
    ...
    const ph = document.createElement('div'); ph.className = 'painting-placeholder';
    ph.innerHTML = `<div class="ph-spinner"></div><div class="ph-title">${p.title}</div>`;
    fw.appendChild(ph);

    const img = document.createElement('img'); img.className = 'painting-img'; img.alt = p.title;
  ...
  function updateStack() {
    cards.forEach(({ el }, i) => {
      ...
      if (i === 0) {
        el.classList.add('cur'); el.style.transform = ''; el.style.opacity = '';
        updateMeta(el._p);
      } else {
        el.style.transform = 'translateY(110%)'; el.style.opacity = '0'; el.style.transition = '';
      }
    });
  }
  ...
  const T = 48;
  dw.addEventListener('mousedown',  e => start(e.clientY));
  dw.addEventListener('touchstart', e => start(e.touches[0].clientY), { passive: true });
  window.addEventListener('mousemove', e => { if (isDrag) move(e.clientY); });
  window.addEventListener('touchmove', e => { if (isDrag) move(e.touches[0].clientY); }, { passive: true });
  window.addEventListener('mouseup',  end);
  window.addEventListener('touchend', end);

  function start(y) { if (!cards.length) return; isDrag=true; startY=y; diffY=0; cards[0].el.classList.add('drag'); }
  function move(y) {
    ...
    if (cards.length > 1) {
      const prog = Math.min(Math.abs(dy) / (T * 1.5), 1);
  ...
  function end() {
    ...
    cards[0].el.classList.remove('drag'); cards[0].el.style.transition = '';
    if (diffY < -T) goNext();
    else {
      cards[0].el.style.transform = '';
      if (cards.length > 1) { cards[1].el.style.transition=''; cards[1].el.style.transform='translateY(110%)'; cards[1].el.style.opacity='0'; }
    }
  }
  function goNext() {
    if (!cards.length) return;
    ...
    if (pointer < queue.length) {
      ...
      dw.appendChild(el); cards.push({ el, data: queue[pointer] }); pointer++;
    }
    if (!cards.length) { showToast('シャッフルして最初から…'); setTimeout(() => initDeck(), 1200); return; }
    updateStack();
  }
  document.addEventListener('keydown', e => { if (e.key==='ArrowDown'||e.key===' ') goNext(); });
  ...
  bh.addEventListener('click', () => {
    ...
    if (i >= 0) { liked.splice(i, 1); bh.classList.remove('lk'); showToast('解除しました'); }
    else {
  ...
  document.getElementById('btn-filter').addEventListener('click', () => { fd.classList.add('open'); buildFD(); });
  ...
  function buildFD() {
    ...
    const cd = document.createElement('div'); cd.className='fd-cnt'; cd.textContent=`現在 ${cnt} 作品が対象`;
    body.appendChild(cd);
    function sec(label, key, vals) {
      const s=document.createElement('div'); s.className='fds';
      const l=document.createElement('div'); l.className='fdl'; l.textContent=label;
      ...
      s.appendChild(l); s.appendChild(pw); body.appendChild(s);
    }
